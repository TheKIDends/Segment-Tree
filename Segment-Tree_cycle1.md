...
**Cây phân đoạn** **(Segment Tree)** là một cấu trúc dữ liệu rất linh hoạt được sử dụng rất nhiều trong các kỳ thi, đặc biệt là trong những bài toán xử lý trên dãy số.
...
- Chiều cao của cây phân đoạn là $\mathcal{O}(\log{N})$, bởi vì khi đi xuống từ gốc đến lá, kích thước của các đoạn giảm đi một nửa.
...
1. Giá trị được lưu trữ tại mỗi nút của cây phân đoạn. **Ví dụ:** Trong cây phân đoạn lấy giá trị nhỏ nhất, một nút sẽ lưu trữ giá trị nhỏ nhất của các phần tử trong đoạn $[l, r]$ mà nó quản lý.
...
Trong cách cài đặt trên có sử dụng các [phép toán bit](https://vi.wikipedia.org/wiki/Ph%C3%A9p_to%C3%A1n_thao_t%C3%A1c_bit) để tối ưu thêm một chút về mặt thời gian:
- `maxN << 2 = 4 * maxN` *(lí do sử dụng kích thước mảng là $4 \times maxN$ sẽ được giải thích ở phần sau)*
- `l + r >> 1 = (l + r) / 2`
- `id << 1 = 2 * id`
- `id << 1 | 1 = 2 * id + 1`
...
<!-- Cây phân đoạn có $2^k$ nút lá sẽ có tổng số nút bằng $2^{k + 1} - 1$ vì nó là cây nhị phân hoàn hảo.

Bây giờ, xét trường hợp tổng quát, bạn sẽ có các nút lá vô dụng nếu bạn đang sử dụng n tổng quát. 

Do đó, trong trường hợp tệ nhất là ta sẽ có gần $2 \times n$ nút lá vì bạn phải làm tròn đến lũy thừa tiếp theo của hai. 

Nếu n là 2 ^ i + 1 thì bạn phải có 2 ^ (i + 1) nút lá = O (2 * n).

Và như chúng ta đã chứng minh trước đây tổng số nút là ~ 2 * nút lá nên chúng ta có tổng không gian là 2 * 2 * n = O (4n). -->
...
long long get(int id, int l, int r, int u, int v) {
    ...
    int mid = l + r >> 1;
    long long get1 = get(id << 1, l, mid, u, v);
    long long get2 = get(id << 1 | 1, mid + 1, r, u, v);
    return get1 + get2;
}
...
void build(int id, int l ,int r) {
    ...
    build(id << 1, l, mid);
    build(id << 1 | 1, mid + 1, r);
    st[id] = node::merge(st[id << 1], st[id << 1 | 1]);
}
...
node get(int id, int l, int r, int u, int v){
    ...
    int mid = l + r >> 1;
    node g1 = get(id << 1, l, mid, u, v);
    node g2 = get(id << 1 | 1, mid + 1, r, u, v);
...
void build(int id, int l, int r) {
    ...
    st[id] = st[id << 1 | 1];
    for (auto x : st[id << 1]) st[id].insert(x);
}
...
int get(int id, int l, int r, int u, int v, int k) {
    ...
    int mid = l + r >> 1;
    int get1 = get(id << 1, l, mid, u, v, k);
    int get2 = get(id << 1 | 1, mid + 1, r, u, v, k);
    return min(get1, get2);
}
...
*Tham khảo code chuẩn tại [đây](https://ideone.com/kgbSSU).*
...
- Mỗi nút của $Segment \space Tree$ sẽ lưu thêm một giá trị $lazy[id]$ với ý nghĩa là tất cả các phần tử trong đoạn $[l, r]$ mà nút $id$ quản lý đều được tăng thêm $lazy[id]$. Nói cách khác, nó sẽ lưu trữ các giá trị cần cập nhật trên mỗi nút.
...
Nhưng thuật toán trên không đủ tốt đối với số lượng truy vấn lớn. Do đó, ta cần một cấu trúc dữ liệu có thể hỗ trợ các hoạt động trên trong thời gian thích hợp.
...
string s;
int cnt[30];
int st[30][maxN << 2], lazy[30][maxN << 2];
...
void build(int id, int l, int r) {
    ...
    for (int ch = 0; ch <= 25; ++ch)
        st[ch][id] = st[ch][id << 1] + st[ch][id << 1 | 1];
}
...
void fix(int id, int l, int r, int ch) {
    ...
    if (l != r){
        ...
        lazy[ch][id << 1] = lazy[ch][id]; 
        lazy[ch][id << 1 | 1] = lazy[ch][id];
    }
    lazy[ch][id] = -1;
}
...
void update(int id, int l, int r, int u, int v, int val, int ch) {
    ...
    int mid = l + r >> 1;
    update(id << 1, l, mid, u, v, val, ch);
    update(id << 1 | 1, mid + 1, r, u, v, val, ch);
    st[ch][id] = st[ch][id << 1] + st[ch][id << 1 | 1];
}
...
int get(int id, int l, int r, int u, int v, int ch) {
    ...
    int mid  = l + r >> 1;
    int get1 = get(id << 1, l, mid, u, v, ch);
    int get2 = get(id << 1 | 1, mid + 1, r, u, v, ch);
    return get1 + get2;
}
